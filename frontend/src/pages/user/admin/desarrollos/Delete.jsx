...
export function DesarrollosDelete({ closeAction, id }) {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const userLogin = useSelector((state) => state.userLogin);
  const { loading, error, userInfo } = userLogin;
  const adminDesarrollosDetails = useSelector(
    (state) => state.adminDesarrollosDetails
  );
  const {
    loading: loadingDetails,
    error: errorDetails,
    desarrollosDetails,
  } = adminDesarrollosDetails;
  const adminDesarrollosDelete = useSelector(
    (state) => state.adminDesarrollosDelete
  );
  const {
    loading: loadingDelete,
    error: errorDelete,
    message,
  } = adminDesarrollosDelete;

  useEffect(() => {
    if ((id && !desarrollosDetails) || (id && desarrollosDetails._id !== id)) {
      dispatch(adminDetailsDevelopment(id));
    }
    if (message) {
      navigate("/admin/desarrollos");
    }
  }, [desarrollosDetails, message]);
  const submitHandler = () => {
    dispatch(adminDeleteDevelopment(id));
  };
  const closeHandler = () => {
    navigate("/admin/desarrollos");
  };
        ...
        {loading || loadingDetails ? (
          ...
          <div className="relative max-h-[90%]  w-11/12 overflow-hidden rounded-md bg-white shadow lg:w-1/2 2xl:w-2/6">
            <div className="flex w-full items-center justify-between rounded-t-md bg-blue-gray-800 p-2 px-4 text-white">
              <h2 className="text-2xl  text-white">Eliminar Desarrollo</h2>
              <button
                type="button"
            ...
            >
              <div className="flex flex-col justify-center gap-3">
                ...
                <div className="my-3 flex justify-center">
                  ¿Seguro que deseas eliminar este desarrollo?
                </div>
                <div className="flex justify-center text-2xl">
                  <img
                    className="h-24 w-24 rounded object-cover"
                    src={desarrollosDetails?.images[0]?.url}
                  />
                </div>
                <div className="flex w-full justify-center px-6 text-2xl">
                  ...
                  <div className="mx-3 flex w-1/2 justify-center">
                    <input
                      ...
                      defaultValue={desarrollosDetails?.nombre}
                      disabled
                    />
                  </div>
                </div>
                <div className="flex w-full justify-center px-6 text-2xl">
                  <div className="flex h-12 w-1/2 justify-end">
                    <p className="my-auto">Direccion</p>
                  </div>
                  <div className="mx-3 flex w-1/2 justify-center">
                    <input
                      className="mx-2 flex w-full rounded-md border-2 p-2 text-center font-bold outline-none focus:border-blue-500"
                      defaultValue={desarrollosDetails?.fiscalAddress.address}
                      disabled
                    />
                  </div>
                </div>
...
export default DesarrollosDelete;
