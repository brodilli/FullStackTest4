...
export function OrderList() {
  ...
  const submitSearch = () => {
    ...
    if (keywordSearch.trim()) {
      navigate(`/admin/ordenes/search/${keywordSearch}`);
    }
  };
  ...
  const submitSort = (field) => {
    ...
    if (keywordSearch) {
      navigate(
        `/admin/ordenes/search/${keywordSearch}/sort/${newSort}/order/${newOrder}`
      );
    } else {
      navigate(`/admin/ordenes/sort/${newSort}/order/${newOrder}`);
    }
  };
  const newSize = (size) => {
    setPageSize(size);
  ...
  useEffect(() => {
    ...
    if (!success && !errorOrders) {
      dispatch(
        adminListOrders(keywordSearch, pageNumber, pageSize, sort, order)
      );
    }
    if (sort && order) {
      setSortField(sort);
    ...
    } else if (link[link.length - 1] === "exportar") {
  ...
  }, [countView, search, orders, id, keyword, location]);
          ...
          <div className="flex items-center">
            ...
            <div className="flex w-full flex-wrap items-center justify-end  gap-3">
              <SearchBox
                ...
                searchObject="orden"
                emptyRoute="/admin/ordenes"
                userType="Admin"
                prevSearch={prevSearch}
                setPrevSearch={setPrevSearch}
                constant={ORDER_ADMIN_LIST_RESET}
              />
              <Link to="/admin/ordenes/crear" className="justify-end">
                <Button
                  variant="gradient"
                  color="white"
              ...
              <Link to="/admin/ordenes/exportar" className="justify-end">
                <Button
                  variant="gradient"
                  color="white"
                  className="hover:bg-blue-800 hover:text-white"
                    ...
                    (
                      ...
                      return (
                        <tr key={orderId}>
                          ...
                          <td className={className}>
                            <Link to={`/admin/ordenes/${_id}/status`}>
                              <Typography
                                as="a"
                                href="#"
                            ...
                            <Link to={`/admin/ordenes/${_id}/eliminar`}>
                              <Typography
                                as="a"
