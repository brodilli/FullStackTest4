...
import {
  adminDeleteMarketplace,
  adminDetailsMarketplace,
} from "../../../../actions/marketplaceActions";
import Loader from "../../../../components/Loader";

export function MarketplaceDelete({ closeAction, id }) {
  ...
  const adminMarketplaceDelete = useSelector(
    (state) => state.adminMarketplaceDelete
  );
  const {
    loading: loadingDelete,
    error: errorDelete,
    message,
  } = adminMarketplaceDelete;

  useEffect(() => {
    if ((id && !marketplaceDetails) || (id && marketplaceDetails._id !== id)) {
      dispatch(adminDetailsMarketplace(id));
    }
    if (message) {
      navigate("/admin/planDePagos");
    }
  }, [marketplaceDetails, message]);
  const submitHandler = () => {
    dispatch(adminDeleteMarketplace(id));
  };
  const closeHandler = () => {
    navigate("/admin/planDePagos");
  };
        ...
        {loading || loadingDetails ? (
          ...
          <div className="relative max-h-[90%]  w-11/12 overflow-hidden rounded-md bg-white shadow lg:w-1/2 2xl:w-2/6">
            <div className="flex w-full items-center justify-between rounded-t-md bg-blue-gray-800 p-2 px-4 text-white">
              <h2 className="text-2xl  text-white">Eliminar Plan de pago</h2>
              <button
                type="button"
            ...
            >
              <div className="flex flex-col justify-center gap-3">
                ...
                <div className="my-3 flex justify-center">
                  ¿Seguro que deseas eliminar este plan de pago?
                </div>
                ...
                <div className="flex w-full justify-center px-6 text-2xl">
                  ...
                  <div className="mx-3 flex w-1/2 justify-center">
                    <input
                      ...
                      defaultValue={marketplaceDetails?.name}
                      disabled
                    />
                  </div>
                </div>
